...
```
        ...
        ┌─────────────────┼─────────────────┐
        ▼                 ▼                  ▼
   对标表.md          STAR库.md         行为画像.md
  JD↔我的证据       4-6条核心经历      行为特质画像
  ·缺口+策略        ·S/T/A/R骨架       ·核心行为特征
                    ·Use for标签       ·JD行为匹配度
                    ·数字准备          ·可能触发追问
                    ·角色边界
        │                 │                  │
...
│       │   ├── C端产品经理.md
│       │   └── 模型策略产品经理.md
│       ├── Agent-Harness-PM/    岗位产物目录（一个岗位一个目录）
│       │   ├── JD人才画像.md    产物① /JD解码
│       │   ├── 对标表.md         产物② /知己
│       │   ├── STAR库.md         产物② /知己
│       │   ├── 行为画像.md       产物② /知己
...
| `/知己` | 收集个人材料 → 逐条对标 → 生成 STAR 库 + 行为画像 | ② 对标表 + STAR库 + 行为画像 |
| `/简历定制` | 基于对标表，Drafter-Reviewer 双Agent 生成定制简历 | ③ 定制简历 |
| `/面试准备` | 匹配 STAR库 + 行为画像 → 预测问题 + 模拟面试 | ④ 面试准备册 |
| `/简历导出PDF` | 将 `定制简历.md` 等导出为 PDF | — |

> **产物链**：`/JD解码` → `/知己` → `/简历定制` + `/面试准备` →（可选）`/简历导出PDF`
>
> **`/知己` 是唯一的信息收集点。** 对标表、STAR库、行为画像在 `/知己` 中一次性生成，后续 `/简历定制` 和 `/面试准备` 只是对同一份数据的消费，不再要求候选人重复提供个人信息。

### 各命令设计要点

**`/简历定制`** — Drafter-Reviewer 双Agent 工作流：
- Drafter 基于对标表起草简历草稿
- Reviewer（独立 Agent，全新上下文）从面试官视角审查：关键词覆盖、经历排序、STAR 完整性、缺口诚实度、红旗预警
- 反馈分 Part A（精确 old_string→new_string，机械应用）+ Part B（叙事建议，人工判断）
- Drafter 逐条应用反馈，拒绝任何编造建议

**`/面试准备`** — STAR 复用 + 行为画像驱动：
- 从 `/知己` 产出的 STAR 库中匹配故事，只做 JD 化微调，不从零推导
- 读取行为画像生成个性化素质题预测
- 新增一致性检查：对标表 ↔ 定制简历 ↔ STAR库 三方交叉核对
- 按面试阶段（初筛/技术面/终面）调整准备册侧重点
- 生成准备册后可选择当场模拟面试
...
| `共享知识/面试方法/03 JD解读方法.md` | `/JD解码`、`/面试准备` |
| `共享知识/面试方法/知识库维护指引.md` | 人工维护 |
| `面试官/招聘配置/面试评价标准.md` | `/JD解码`、`/面评生成`、`/面试准备`、`/知己` |
| `面试官/招聘配置/岗位人才画像.md` | `/简历定制`、`/知己` |
| `面试官/招聘配置/岗位信息.md` | `/面试准备` |
| `面试官/招聘配置/面试规划和时间控制.md` | 面试官参考 |
| `面试官/招聘配置/面试官信息.md` | 面试官参考 |

> 每个知识文件头部均有 `<!-- 消费者: ... -->` 注释标注，可快速定位。
...
```text
# 候选人侧（推荐顺序）
/JD解码 @候选人/幻方-DeepSeek/JD原始/Agent Harness 产品经理.md
/知己 @候选人/幻方-DeepSeek/Agent-Harness-PM/JD人才画像.md @候选人/我的材料/个人简历.md
/简历定制 @候选人/幻方-DeepSeek/Agent-Harness-PM/对标表.md
/面试准备 @候选人/幻方-DeepSeek/Agent-Harness-PM/对标表.md
/简历导出PDF @候选人/幻方-DeepSeek/Agent-Harness-PM/定制简历.md
